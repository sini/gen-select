...
| Constructing intensional functions (`mkIntensional`) and the identity-regime discipline underneath `selectorEq` (`identityOf`/`comparisonSubject`/`conservativeEq`) — gen-select imports these from gen-algebra rather than vendoring them (`lib/constructors.nix`'s own header: "THE IDENTITY-REGIME DISCIPLINE IS IMPORTED, NOT VENDORED"); the file's only local logic is `selectorEq`'s own tag dispatch (`entity`/`coord`/`when`/structural fallback), which calls `algebra.conservativeEq` on the `when` branch. This replaces an earlier claim that gen-select held private `identityOf`/`comparisonSubject` bindings — true only before the vendored copy was retired | `gen-algebra` — "gen-algebra: pure Nix algebra — search monad, records, intensional functions, either" |
